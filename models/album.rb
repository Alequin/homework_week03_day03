...
class Album
  ...
  def delete()
    sql_command = "DELETE FROM albums WHERE id = #{@id}"
  ...
  def Album.find(id)
    sql_command = "SELECT * FROM albums WHERE id = #{id}"
    album_details = SqlRunner.run(sql_command)[0]
    return Album.new(album_details)
  end
